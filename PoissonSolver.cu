void __global__ kernel(double *u,double *f,double *unew,int nx,int ny,double dx,double dy)
{
  ...
  if((((i==0)||(j==0))||(i==(nx-1)))||(j==(ny-1))) {
    unew[i*11+j]=f[i*11+j];
  }else {
    unew[i*11+j]= 0.25 * (u[(i-1)*11+j] + u[i*11+j+1] + u[i*11+j-1] + u[(i+1)*11+j] + f[i*11+j]*dx*dy);
  }
  //if (i<11 && j<11)
  //unew[i*11+j] = u[i*11+j];
}
# include <stdlib.h>
# include <stdio.h>
# include <math.h>
...
void sweep(int nx,int ny,double dx,double dy,double f[11UL][11UL],double u[11UL][11UL],double unew[11UL][11UL]) {
  ...
  cudaMalloc((void **) &device_u, sizeof(us));
  cudaMalloc((void **) &device_f, sizeof(fs));
  cudaMalloc((void **) &device_unew, sizeof(uns));

  //Copy from host to device
  cudaMemcpy(device_u, us, sizeof(us), cudaMemcpyHostToDevice);
  cudaMemcpy(device_f, fs, sizeof(fs), cudaMemcpyHostToDevice);
  cudaMemcpy(device_unew, uns, sizeof(uns), cudaMemcpyHostToDevice);
  ...
  cudaMemcpy(us, device_u, sizeof(us), cudaMemcpyDeviceToHost);
  cudaMemcpy(fs, device_f, sizeof(fs), cudaMemcpyDeviceToHost);
  cudaMemcpy(uns, device_unew, sizeof(uns), cudaMemcpyDeviceToHost);
  cudaFree(device_u);
  cudaFree(device_f);
